---
name: headed-browser
description: Run a HEADED (visible) browser on a shared virtual display that the user watches live from their phone via noVNC. Use whenever a browser must run non-headless — Playwright/Selenium/nodriver/raw Chromium automation, web scraping that needs a real display to defeat anti-bot (DataDome, PerimeterX, Cloudflare interactive), or when the user asks to SEE/watch the browser while testing or debugging a flow remotely. Pairs with the webapp-testing skill: that one drives Playwright headless for Claude's own screenshots; this one makes the browser headed and visible to the human. Part of the Remoteclaude remote-dev setup (display container on DISPLAY :99, noVNC on :6080).
---
