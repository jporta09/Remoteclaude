...
- **El servidor X del visor (`-ac`).** El escritorio ya pide password (VncAuth), pero el
  servidor X sigue con control de acceso desactivado: un proceso local del host puede
  conectarse al `:99` por el 6099 y leer o inyectar eventos. Sacarlo obliga a repartir la
  cookie de X al navegador *headed*, que corre fuera del contenedor; queda pendiente.
