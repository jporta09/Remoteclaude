...
| 3 | DG-2 | Dos verdes: los SVG oficiales del manual (ISO/001) usan `#42A648`; `colors.xml marvin_green` y toda la UI `#71BF44` (p06 del manual). En el header de hosts conviven (iso vs titulo) | DECIDIDO (dueno, 2026-09-03): canonico = `#71BF44` (el del manual y la UI); re-exportar/tintar los activos derivados de los SVG |
...
| 2 | SRE-5-2 (op) | `marvin-doctor` con jq roto o RFC3339Nano dice "✅ vence en ? dia(s) (con margen)" = falso OK (`${dias:-?}` cae al else). Y el doctor DESPLEGADO en el host real (`~/.local/bin/marvin-doctor`, 2026-08-23) difiere del repo y no tiene el bloque de expiry: el fix SRE-4p-1 host-side de la 4a NUNCA se desplego | El else no dice "con margen" si el parseo fallo (desconocido); DECIDIDO (2026-09-03): re-correr `setup-host.sh` |
...
| 2-3 | A5-3 | El OAuth client (scope "Auth Keys write + tag", README:49-50) vive en el `.env` del repo del host, que `ts-link-qr.sh:33-36` sourcea = dominio de confianza del agente; y la skill (`remotemarvin/SKILL.md:50-51`) le ordena a Claude correr `ts-link-qr.sh --png`. Un Claude inyectado no necesita ganar la carrera del QR (SEC5-2): mintea con `reusable:true` + tag -> nodo del atacante DURABLE (tag = sin expiry) con ACL default all-to-all -> lateral a toda la tailnet (ASI03). Existia desde F10; cuatro pasadas miraron solo el celular | DECIDIDO (dueno, 2026-09-03): sacarle a la skill la capacidad — Claude pide al usuario que corra el script en su terminal (README, manual y guia rapida ya eran manuales; la orden a Claude fue una redaccion del 27-08, commit 5f85c46). Secret por TTY/keyring y ACL minima quedan como mejoras |
