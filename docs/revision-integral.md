...
| **1** | **◑ PARCIAL · Ring buffer no sobrevive a la muerte del proceso** → sin post-mortem de crash/OOM/reboot. Los **cuelgues de UI SÍ se persisten** a disco (`VigiaUi`, v1.15.0: al reabrir se re-cargan al Diagnóstico) — falta persistir también los eventos de conexión. | SRE | `Diagnostico.kt`, `VigiaUi.kt` |
...
- **#2 — la notif "Claude te espera" sólo llega al VOLVER a la app, no en background.** Android estrangula el hilo del `tail -F` con la app atrás → la línea del hook se lee recién al resumir. **Fix pendiente: foreground service opcional** (mantiene el canal vivo). Confirma que el techo "app muerta/atrás" no es opcional si se quiere que la alerta sirva.
- **#1 — la app se "traba" al volver desde otra app, en Claude; a veces. DIAGNOSTICADO 2026-08-21: NO es un cuelgue de UI, es la CONEXIÓN.** El `VigiaUi` (v1.15.0) hizo su trabajo: cuando el usuario lo reprodujo en el celu real, **NO** registró ningún `cuelgue-ui` → el hilo principal NUNCA estuvo colgado. Lo que sí mostró el Diagnóstico: **5 reconexiones** en ~18 min (`conectado` repetido), y el freeze terminó justo en la reconexión. O sea: en background el hilo se estrangula → el keepalive no manda → la conexión SSH **se cae**; al volver, el loop está en **backoff** (espera hasta 8 s) y **nadie lo despertaba**, así que la terminal quedaba muda/"trabada" esa espera. Mismo root que #2 (estrangulamiento de fondo). **Fix v1.15.2 (`reintentarConexionYa` race-safe + kick en `onResume`):** al volver a primer plano se rompe el backoff y reconecta al instante. Barato, cero batería. El **foreground service** (opt-in) sigue siendo el fix completo para no caerse en background (arregla #1 y #2 de raíz). El `VigiaUi` se queda igual (buen negativo: descartó el ANR).
