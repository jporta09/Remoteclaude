...
- **#1 — la app se traba al volver desde otra app, en Claude; a veces.** Es PREVIO a v1.14.0 y **NO** es el modo lectura (pasa sin prompt). **No se pudo forzar** (emulador + celu real S23+ por adb: flood alt-screen y Claude real + scroll → resume limpio, sin ANR/jank), así que se descarta la "ráfaga/acumulación". **v1.15.0 agrega `VigiaUi`**: vigila el hilo principal en primer plano y, si se cuelga ≥4 s, **captura su stack** al Diagnóstico (F8, Compartible) + a disco (sobrevive al force-close). Cuando el freeze pase en uso real, queda grabado DÓNDE — sin adb ni reproducirlo. E2E `vigiaUi_registraElCuelgueDelHiloPrincipal`. Hipótesis viva: caída-de-conexión-y-reconexión-en-resume (redibujo tmux de golpe), a confirmar con el stack.
