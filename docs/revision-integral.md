...
| **Subir documentos desde el celu** (pedido ago-2026, para un próximo upgrade) | Hoy Documentos es de una sola vía: el host comparte con `marvin-share` y el celu mira. Falta la vuelta: una zona para dropear/elegir archivos del teléfono (imágenes, archivos) que viajen por SSH a un lugar dedicado del host (p. ej. `~/RemoteMarvinDocs/subidos/` o similar, separado de lo que comparte Claude). Piezas ya existentes para apoyarse: `RemoteControl` (canal SSH con quoting y pinning), el tope de 8 MB del visor como referencia de tamaño, y la validación de nombres de `marvin-show.sh`. A definir: picker de Android (SAF) vs drag & drop, y cómo lo ve Claude del otro lado. |
| **Ordenar la lista de documentos** (pedido ago-2026, mismo upgrade) | Criterios: nombre, tamaño, tipo, fecha de creación y fecha de modificación. Hoy la lista sale en el orden que devuelve el `find` del host y no se puede cambiar. Ojo con "fecha de creación": el `find -printf` actual solo trae mtime; la creación (`%W`/statx btime) depende del filesystem del host — si no está disponible, degradar a mtime o esconder ese criterio. La UI puede ser un botón junto al ⟳ que cicle/despliegue el criterio, persistido en prefs. |
| **Borrar documentos desde el celu** (pedido ago-2026, mismo upgrade) | Long-press sobre la tarjeta → confirmar → `rm` por SSH del archivo en `~/RemoteMarvinDocs`. Reusar el mismo saneo de nombres de `readDocBase64` (nada de comillas/saltos/rutas) para que el nombre jamás se interpole al shell, y un E2E de regresión estilo `unNombreConComillaNoEjecutaNada`. |
