...
- **Prompts de decisión en el celu (ASI09) — DECISIÓN 2026-08-20 (opción B, alerta por hook):** el problema de origen es **aprobar sin leer** (con el teclado arriba el pty es 46×10 y el prompt colapsa a "1. Sí/2. No"). **Claude SÍ emite señal**: el hook **`Notification`** con `notification_type:"permission_prompt"` (confirmado en el binario `2.1.237` + docs oficiales; local, **token-free**, fiable sobre SSH/tmux; ~6 s de latencia en terminal). Se resuelve **por el hook**, NO por catcheo de texto ni re-render: (a) se **REMUEVE la hoja de aprobación (A1) y la notif F7** (`Aprobacion.kt`); (b) se lee el prompt crudo; (c) la alerta de "Claude te espera" se reimplementa sobre el hook. Encuadre de producto: la app **degrada a SSH+tmux sin Claude** pero **integra de primera con Claude** (los hooks entran perfecto) — no es "agnóstica". *Ya en el repo, sujeto al fork abierto:* "modo lectura" visual (`hayPromptDeOpciones` + `PromptDeOpcionesTest`, commit `57f12bd`) baja el teclado ante ≥2 opciones numeradas (verificado en vivo, 46×10→46×25); **fork:** como el hook tiene ~6 s de lag, decidir si el disparo instantáneo en foreground se queda con la heurística visual (150 ms) o todo va por hook.
