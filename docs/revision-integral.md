...
| **1** | **⏳ `marvin-share.sh` copia con el basename crudo** (`cp -- "$f" "$DEST/"`): el nombre hostil que el fix de arriba cierra en la app **puede nacer acá** (el otro productor, `marvin-render.py`/`safe_name`, sí valida con regex+allowlist). Defensa-en-profundidad del lado del PRODUCTOR (la frontera de la app ya quedó cerrada; Claude tiene shell igual). Endurecer = tarea del `remediador-seguridad`. | Seguridad (nota) | `plugins/remotemarvin/scripts/marvin-share.sh` |
