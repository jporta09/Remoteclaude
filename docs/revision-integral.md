...
| **2** | **✅ RESUELTO (v1.18.0) · OSC52 sin mostrar el contenido**: el toast atribuido decía cuántos caracteres pero no QUÉ. **Cerrado** con preview **no intrusivo**: el toast (<20KB) incluye un snippet del contenido y el diálogo de confirmación (≥20KB) muestra un preview truncado. Helper `previewSeguro` (colapsa control/espacios, trunca) top-level, unit `PreviewSeguroTest`. Verificado e2e (OSC52 real → toast con el contenido). | Seg, Arq-IA | `TerminalClients.kt` (`onCopyTextToClipboard`, `previewSeguro`) |
...
| **1** | **⏳ menores**: `vncPassword()` mudo → degradación (riesgo de auth **REFUTADO**); **✅ `DictationController.vivo` ahora `@Volatile`** (v1.18.0); **✅ el dictado ya no se pierde mudo si se cierra el tab destino** → avisa por Toast en vez de escribir a una sesión muerta (v1.18.0); **✅ "Limpiar" del diagnóstico ahora pide confirmación** (v1.17.0); quickstart sin URL/one-liner; **✅ `flag-subidos-context.sh` sobre-disparaba** ante cualquier Bash que mencione la ruta → ahora sólo ante comandos de LECTURA (cat/grep/head/…); **✅ `quitar_bloque_sentinelas` borraba a EOF si faltaba `end`** → ahora exige la sentinela de fin y si no está no toca el archivo (evita data-loss en teardown). | varios | `flag-subidos-context.sh`, `setup-host-lib.sh` |
