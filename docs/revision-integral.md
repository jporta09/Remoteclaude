...
| 4 | **✔ RESUELTO (WS-F)** · **La pantalla de aprobación de Claude colapsa en el celu (ASI09)**: con teclado arriba el pty es 46×10; el diff se empuja fuera y solo sobreviven "1. Yes / 2. No". Ahora un watcher lee el prompt del buffer (spike: `transcriptText` captura la pantalla alternada del fullscreen; se usa la variante sin unir líneas) y levanta una **hoja de aprobación** nativa: el comando/diff completo scrollable + botones grandes que inyectan la elección con `session.write`. Unit del parser + E2E de round-trip (el host recibe la elección) ✅ | Arq. IA (cuantificado), Dev (lo anticipó) | `Aprobacion.kt`, terminal |
...
| 2 | **✔ RESUELTO (F7)** · **Sin notificación cuando Claude está bloqueado esperando aprobación** (componía ASI09: aprobabas tarde/apurado o te lo perdías): con la app en segundo plano, cuando el parser detecta un prompt de aprobación se postea una **notificación** (canal `IMPORTANCE_HIGH`, permiso `POST_NOTIFICATIONS` pedido en Android 13+); tocarla trae la app al frente (singleTop) y ahí aparece la hoja. El aviso se baja al volver, al responder o si el prompt se va. **Versión liviana**, sin foreground service (mantener el SSH vivo en background sigue siendo by-design aparte). E2E: en segundo plano avisa por notificación y NO abre la hoja; al volver se baja el aviso y la hoja aparece ✅ | SRE, Arq. IA | `Aprobacion.kt` (notificar/alPrimerPlano), `MainActivity` (onResume/onPause) |
...
| **2** | **⏳ Hoja de aprobación: color sin semántica + no valida autenticidad** (ASI09; re-revisión: bajado de sev-3 para ser coherente con el score ASI09=Medio del cierre). **Dos mitades:** (a, CONCRETO y barato) todos los botones el mismo verde — No indistinguible de Yes/"don't ask again"; (b, PALANCA de diseño) texto arbitrario de terminal que matchee `"Do you want"` + opciones sintetiza una hoja "oficial" y el diff es verbatim del buffer (spoofeable): la hoja es transporte fiel, no validación. (a) es fix limpio; (b) es **decisión de diseño, no fix autónomo.** | Arq-IA, UX | `Aprobacion.kt:61,82-83,183` |
...
- **Fila 554 (notif de aprobación, F7) → ◑ PARCIAL** (re-revisión: bajada de ✅). El wiring y el PendingIntent están bien, pero la notif sólo se postea con el **proceso vivo**; con el proceso muerto en Doze/LMK (by-design, sección B) **el escenario que la MOTIVÓ** —estás lejos del celu y Claude se traba— queda mudo, y la notif *stale* muestra el prompt viejo (benigno: al tocarla re-lee la pantalla viva). Composición de dos decisiones by-design → candidato a **foreground service opcional**.
...
- **Legibilidad de prompts de decisión (ASI09) — camino Claude-independiente agregado (`57f12bd`):** "modo lectura" — cuando el buffer VISIBLE termina en un prompt con opciones numeradas al pie (`≥2` líneas tipo `1) Yes / 2) No`, cursor `❯`/`>` opcional; genérico, no atado al box de Claude), la app **baja el teclado** una vez (el pty pasa de 46×10 a 46×25 → recuperás contexto para leer antes de elegir); tocar la terminal reingresa a modo escritura. Acotado a ESE caso (no a cualquier salida grande — para eso está scroll/pinch), debounced 150 ms, de bajo riesgo (sólo decide CUÁNDO bajar el teclado; no re-renderiza ni inyecta). Verificado en vivo en el emulador (prompt de opciones baja / `seq 1 40` no) + `PromptDeOpcionesTest`. Es la dirección "leer el prompt crudo como contenido de terminal" que el usuario prefiere sobre re-renderizar con autoridad de marca → **baja la prioridad de la palanca F.4.1** (endurecer la hoja de aprobación, hoy *casi descartada*). **Pendiente de verificar contra Claude real:** cómo convive con el watcher de la hoja de aprobación (`Aprobacion.kt`) cuando AMBOS disparan sobre el mismo prompt.

### F.4 · Palancas de mayor ROI que quedan (del cierre del Arq-IA)
1. **Endurecer la hoja de aprobación** (ASI09/ASI05) — *casi descartada* (el usuario prefiere leer el prompt crudo; ver "modo lectura" en F.3): diferenciar color de botones (No neutro, "don't ask again" ámbar); anclar la detección a la estructura del box real; señalizar contenido no verificado.
