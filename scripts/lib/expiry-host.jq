...
if .Self == null then "SINSELF" elif .BackendState != "Running" then "NORUN" elif (.Self.Tags // [] | length) > 0 then "TAGGED" elif .Self.KeyExpiry == null then "NOEXP" else (((.Self.KeyExpiry | sub("\\.[0-9]+"; "") | fromdateiso8601) - now) / 86400 | floor | tostring) end
